...
def get_overview_stats(db: Session = Depends(get_db)):
    ...
    if open_incidents:
        ...
        risk_band = (
            "CRITICAL" if avg_risk >= 75
            else "HIGH" if avg_risk >= 50
            else "MODERATE" if avg_risk >= 25
            else "LOW"
        )
        system_status = "ALERT" if avg_risk >= 75.0 else "ELEVATED" if avg_risk >= 50.0 else "HEALTHY"
        trend = [
            {"time": "08:00", "score": round(max(0, avg_risk - 15), 1)},
            {"time": "10:00", "score": round(max(0, avg_risk - 10), 1)},
            {"time": "12:00", "score": round(max(0, avg_risk - 5), 1)},
            {"time": "14:00", "score": round(max(0, avg_risk - 2), 1)},
            {"time": "16:00", "score": round(avg_risk, 1)}
        ]
    else:
        avg_risk = 0.0
        risk_band = "LOW"
    ...
    avg_readiness = round(total_r / max(1, len(backups)), 1) if backups else 85.0
    ...
    return {
        ...
        "organization_risk_band": risk_band,
